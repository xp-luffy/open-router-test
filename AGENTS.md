...
A lean lead-tracker for founders with a built-in paid tier — capture leads, track pipeline status, and unlock full access via Stripe checkout from day one.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "285927607+xp-luffy@users.noreply.github.com"
  git config user.name "xp-luffy"
  ```
